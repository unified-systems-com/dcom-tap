...
**dcom** — *design · configuration · operation*. One axis, three positions, for anything a designed system
produces. This plugin puts that axis on the grid as a vocabulary other plugins inherit.
...
| Boot records | `ci` (in-package): installs this plugin alone, seeds the pack, and is the stack the plugin's own tests run in. |
| Spec home | This file, in this repository, from the first commit. Core is never edited to define dcom. |

**Default dimensions** (on every node this plugin ships)

| Dimension | Value | Why |
| --- | --- | --- |
| `tap.meta` | `dimension` | Core's `Dimension` model stamps it; the pack's nodes are the dictionary, not entries in it, and carry no `dcom` value themselves. |
...
that opts in. It is not universal: a system that was never designed — a person, a place — has no
configuration and no operation, and a plugin that models such things leaves the axis alone. For everything
TAP collects from designed systems, it is the first question to answer.

Two properties follow from the axis and are the reason it exists as a shared vocabulary rather than a
...
- **Only configuration goes stale.** A fact derived from the *content* of a configuration node — a
  ...
  never needs re-checking, because operations do not change. The axis is how the grid knows which
...
- **One query reads a whole side of the grid.** "Everything that is declared" and "everything that
  happened" are each a single containment predicate across every plugin that inherits the axis, with no
  enumeration of types.

The axis is deliberately small: three values, one key, four nodes. Everything else a reader needs — what a
...
| 2. | Legible to Player 3 | The axis and each value are nodes on the grid with descriptions written for an AI reader, so the vocabulary is read from the grid, never inferred from a type name. |
| 3. | Derivable, not stored | Parent, children and the label a node defines are all derived from its name. Nothing about the hierarchy is stored twice. |
| 4. | Opt-in | Inheriting the axis is one manifest line; a plugin that does not inherit it is untouched. |
| 5. | Permissive first | v0 defines the rules and ships the pack; enforcement of stamping is deferred until the rules have been used in the field (`Out of Scope`). |
...
| --- | --- | --- |
| `dcom` | axis | The design / configuration / operation axis. Every node or edge that describes a designed system may carry one `dcom` label saying which kind of fact it is: `design` (intent, edited), `configuration` (declared to the system, versioned, changes under you), or `operation` (happened, immutable). An entity carries one value or none, never two. The value describes the observation, not the thing: a pipeline's definition is configuration; each run of it is an operation. Only facts derived from configuration can go stale, because only configuration changes. The three values are the nodes named `dcom.design`, `dcom.configuration` and `dcom.operation`. |
| `dcom.design` | value | The `design` position on the dcom axis: intent that precedes any concrete instance — what a system is meant to do, as authored by people. A design is edited, not versioned by the system it describes; a rewrite supersedes it. Nothing the system reads at run time is a design; the moment a design is encoded where the system reads it, that encoding is `configuration`. |
| `dcom.configuration` | value | The `configuration` position on the dcom axis: a declared instance that a system reads to decide what may happen — a definition file, a job as declared, a protection rule, a selector for where work may run. Configuration has history: it changes under you, each change is a new version of the same node, and any fact derived from the content of an older version (a reference it contained, a line it flagged, a permission it granted) may no longer hold. When you ask "is this still true?", ask it of configuration. |
| `dcom.operation` | value | The `operation` position on the dcom axis: something that happened — a run, an execution, a scan, a finding emitted at a moment in time. An operation is immutable once recorded; it can be observed again but never changed, and a later occurrence is a later operation. Its status or conclusion are fields on it, not a change to it. Facts derived from an operation never go stale. When you ask "did this happen, and how did it end?", ask it of an operation. |
...
of the bundle, and the description text is edited in place under a new batch so the ids stay stable.
...
| Fact | Derivation |
| --- | --- |
| Parent of a node | Its name with the last dot-separated segment removed. `dcom.operation` → `dcom`. A name with no dot is an axis and has no parent. |
| Values of an axis | Every dimension node whose name starts with the axis name followed by a dot. |
...
| Depth | v0 defines one level: axis and value. Deeper names are not forbidden by the model but are not used by this pack and carry no defined meaning. |

One constraint makes the derivations unambiguous: **a value segment never contains a dot.**

This rule is stated here because dcom is the first dimension pack, and the next axis anyone defines should
...
reason. Silence is not exemption; it is an omission, and v0 tolerates it (`Goals` 5) without pretending it is
a decision.

The plugin's own spec gains a **dcom** section listing every node and edge type with its value or its
exemption. That table is the plugin's statement of conformance until a check exists to derive it.
...
| 6 | **Dictionary nodes carry no value.** The four pack nodes, and any future dimension node, are stamped `tap.meta: dimension` and nothing from the axis they define. |
...
| --- | --- | --- |
| `design` | Changes by being rewritten; the system does not read it. | Nothing in v0. Named so the axis is complete and so a design-side plugin has somewhere to land. |
...
or a cache that is wrong. The promise is the reason to get the value right, and the reason `Out of Scope`
names enforcement as the next thing to build.
...
The queries a reader — a panel, a test, an AI assistant — uses to read the axis. Each runs against the
...
| What the axis means | `MATCH (d:dimension) WHERE d.name = "dcom" RETURN d` |
...
The last row is how conformance is *read* before any check *enforces* it: an inheriting plugin's test may
...
The in-package `ci` boot record installs this plugin alone (it has no dependencies), seeds the pack, and is
the stack the plugin's own tests run in. Offline, credential-free, no network.

Tests (`tap_plugin/dcom/tests/`):

| Test | Proves |
| --- | --- |
| `test_dcom_manifest.py` | Identity chain (slug, dist, namespace, entry point) agrees; no `[models]`, no `[edges]`, one `[grift]` entry. |
...
- **Mandatory stamping.** No manifest entry declares which dimensions a plugin's types must carry, and no
  build or boot step refuses an unstamped type. v0 is permissive on purpose: the shape of that entry should
  be dialled in after the rules have been used, not before. The likely home is a manifest field consumed at
  plugin build time, applying to every type a plugin ships.
- **Conformance tracking over time.** Whether a plugin that declares dcom has in fact stamped every type is
  readable today (`req-dcom-queries`, last row) and not yet computed or reported anywhere.
- **Edges between dimension nodes.** Redundant with the naming rule; not shipped.
- **A `design` collector.** Nothing on the grid carries `design` yet. The value is defined so the axis is
  whole and so the first design-side plugin has a place to stand.
- **Deeper hierarchy.** One level, axis and value, is all v0 defines.

## Future

- **Mandatory dimensions in the manifest.** A `[dimensions]` table (or similar) naming the keys every type
  in the plugin must carry; validated when the plugin is built; the dcom section of a plugin's spec becomes
  derived from it rather than authored.
- **Conformance as a three-state report.** Per plugin, per type: stamped / exempt with a reason / omitted —
  computed, never declared.
- **More dimension packs.** Any axis that more than one plugin needs follows this plugin's shape: its own
  spec, its own repo, dotted dimension nodes, no edges, inherited by dependency.
- **Design on the grid.** Requirements and architecture as `design` nodes, joined to the configuration that
  encodes them, so the question "does the configuration still match the design?" becomes a query.
...
| Approved for Development | Reviewed and accepted; ready to implement |
| In Development | Being built |
| Implemented | Built and tested |
| Verified | Observed correct on a running instance |
| Deprecated | Retired |
